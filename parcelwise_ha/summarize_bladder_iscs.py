...
nfiles = glob.glob(os.path.join(bladdir, '*sub*mat'))
...
# TESTING HYPERALIGN ON HALF OF BLADDER, DELETE LATER
# Load all full mat files
mats = []
for sub in range(len(mysubs)):
    f = os.path.join(bladdir, mysubs[sub])
...
for parcel in range(len(indices)):
    ...
    print('Size of data sets: {0}'.format(dss[0].shape))
...
df_results.to_csv(os.path.join(savedir, 'bladder_only_parcelwise_iscs_subs-' + str(nsubs) + '.csv'))
